...
TIMELINE_MODEL_PATH = MODELS_DIR / "Timeline_Risk_model_FIXED.pkl"

if TIMELINE_MODEL_PATH.exists():
    timeline_model = joblib.load(TIMELINE_MODEL_PATH)
else:
    timeline_model = None
...
def predict_timeline(db: Session, task_id) -> dict:
    if timeline_model is None:
        raise HTTPException(
            status_code=503,
            detail="Timeline prediction model is not currently available."
        )
    task = _get_task(db, task_id)
    _require(task, "assigned_to", "priority", "complexity", "team", "estimated_hours")
    user = _get_user(db, task.assigned_to)
