...
const AI_CORRECTIVE_ACTIONS: Record<string, string> = {
  "T-003":
    "High risk delay. Reassign 1 backend developer from low-priority tasks to review JWT token expiration logic.",
  "T-007":
    "Database schema migration issue. Schedule an immediate index benchmarking session and run automated diagnostic check.",
};
...
export default function OverdueTasks() {
  ...
  const [escalationLogs, setEscalationLogs] = useState<
    Record<string, EscalationRecord[]>
  >({});

  const [selectedTaskForEscalation, setSelectedTaskForEscalation] =
    useState<Task | null>(null);

  const [managerInput, setManagerInput] = useState(
    "Vikram Executive (VP Eng)"
  );

  const [escalationNotes, setEscalationNotes] = useState("");

  const [activeHistoryTaskId, setActiveHistoryTaskId] =
  ...
  async function handleConfirmEscalation() {
    ...
    try {
      await escalateTaskMutation.mutateAsync({
        ...
        reason:
          notes.trim() ||
          `Escalation requested for ${managerNotified}.`,
      });
    } catch {
      // Mock fallback
    } finally {
      const newRecord: EscalationRecord = {
        ...
        notes:
          notes ||
          "Direct escalation triggered by project manager.",
      };
  ...
  const riskCounts = useMemo(() => {
    return (overdue || []).reduce(
      (
        ...
        const category =
          riskCategorization(daysOverdue).level;
          ...
          <div className="flex items-center justify-between mb-5">
            <div>
              <h1 className="text-[24px] font-bold text-[#111827]">
                Overdue Management Panel
              </h1>

              <p className="text-[14px] text-[#6B7280] mt-1">
                Real-time overdue tracking, AI corrective guidance,
              ...
              <>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="flex items-center justify-between p-4 rounded-xl bg-amber-50/70 border border-amber-200">
                      <div>
                        ...
                        <p className="text-[28px] font-extrabold text-amber-700 leading-tight mt-1">
                          {riskCounts.low}
                        </p>
                      </div>
                    ...
                    <div className="flex items-center justify-between p-4 rounded-xl bg-orange-50/70 border border-orange-200">
                      <div>
                        <p className="text-[12px] font-bold text-orange-800 uppercase tracking-wider">
                          Medium Risk (7 - 13 Days)
                        </p>

                        <p className="text-[28px] font-extrabold text-orange-700 leading-tight mt-1">
                          {riskCounts.medium}
                        </p>
                      </div>
                    ...
                    <div className="flex items-center justify-between p-4 rounded-xl bg-red-50/70 border border-red-200">
                      <div>
                        <p className="text-[12px] font-bold text-red-800 uppercase tracking-wider">
                          High Risk (14+ Days)
                        </p>

                        <p className="text-[28px] font-extrabold text-red-700 leading-tight mt-1">
                          {riskCounts.high}
                        </p>
                      </div>
                ...
                <div className="space-y-5">
                  {overdue.map((task: Task) => {
                    ...
                    const risk =
                      riskCategorization(daysOverdue);

                    const aiSuggestion =
                      AI_CORRECTIVE_ACTIONS[task.id] ||
                      "AI Suggestion: Re-balance workload and set daily progress check-ins with assignee.";

                    const history =
                      escalationLogs[task.id] || [];

                    return (
                      <div
                        key={task.id}
                        className="bg-white border border-[#E5E7EB] rounded-xl p-5 border-l-4 border-l-red-500 shadow-sm hover:shadow-md transition-shadow"
                      >
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <span
                                className={`text-[12px] font-bold px-2.5 py-0.5 rounded-lg ${risk.style}`}
                              >
                                {risk.level}
                              </span>
                                ...
                                {activeHistoryTaskId ===
                                  ...
                                  <div className="mt-2 space-y-2 bg-gray-50 border border-gray-200 rounded-xl p-3">
                                    {history.map((log) => (
                                      <div
                                        key={log.id}
                                        className="text-[12px] text-gray-700 border-b border-gray-200 last:border-0 pb-1.5 last:pb-0"
                                      >
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-3 bg-red-50 border border-red-200 rounded-xl">
                    ...
                    <p className="text-[14px] font-semibold text-red-900">
                      {selectedTaskForEscalation.title}
                    </p>

                    <p className="text-[12px] text-red-700 mt-1">
                      Assignee:{" "}
                      {selectedTaskForEscalation.assignee}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    >
                      <option value="Vikram Executive (VP Eng)">
                        Vikram Executive (VP Eng)
                      </option>

                      <option value="Ananya Roy (Head of Infra)">
                        Ananya Roy (Head of Infra)
                      </option>

                      <option value="Siddharth Rao (Engineering Director)">
                        Siddharth Rao (Engineering Director)
                      </option>
                    </select>
                  </div>
                  ...
                  <div className="flex justify-end gap-3 pt-2">
                    ...
                    <button
                      ...
                      disabled={
                        escalateTaskMutation.isPending
                      }
